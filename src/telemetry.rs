...
impl Telemetry {
    ...
    pub(crate) async fn send(&self) -> eyre::Result<Response> {
        ...
        let req = http_client.post(TELEMETRY_REMOTE_URL)
...
async fn distinct_id() -> eyre::Result<Uuid> {
    ...
    distinct_id_file
        .read_exact(&mut distinct_id_bytes)
        .await?;
    let distinct_id = String::from_utf8(distinct_id_bytes.clone()).wrap_err("Distinct UUID was not in UTF-8 bytes")?;
    let distinct_id = if distinct_id.is_empty() {
        let distinct_id = Uuid::new_v4();
        tracing::debug!(%distinct_id, "Writing new distinct ID");
        distinct_id_file.write_all(format!("{distinct_id}\n\n{TELEMETRY_IDENTIFIER_DESCRIPTION}").as_bytes()).await?;
        distinct_id
    } else {
